...
def binding_vectors_from_bedpe(bedpe_file, N_beads, region, chrom,
                              normalization=False, viz=False,
                              J_mode="binary",        # NEW
                              J_norm=None,            # NEW: None, "global", "row"
                              alpha=1.0):             # NEW: for distance decay
    '''
    Extended version with flexible J construction.

    J_mode:
        "binary"   -> current behavior (default)
        "strength" -> weighted by loop score
        "distance" -> weighted by exp(-alpha * genomic distance)

    J_norm:
        None       -> no normalization
        "global"   -> divide by sum(J)
        "row"      -> row-normalized
    '''

    df = pd.read_csv(bedpe_file, sep='\t', header=None)
    df = df[(df[1]>=region[0])&(df[2]>=region[0])&
            (df[4]>=region[0])&(df[5]>=region[0])&
            (df[5]<region[1])&(df[4]<region[1])&
            (df[1]<region[1])&(df[2]<region[1])&
            (df[0]==chrom)].reset_index(drop=True)
    ...
    for i in range(len(df)):
        ...
        elif J_mode == "strength":
            weight = df[6][i]

        elif J_mode == "distance":
            loop_size = np.abs(y - x)
            weight = np.exp(-alpha * loop_size)

        else:
            weight = 1.0  # fallback safe
    ...
    # Backbone connectivity (unchanged)
